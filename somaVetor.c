...
typedef struct {
    ...
    long int inicio; //Índice de início do bloco de elementos que a thread irá processar
    long int fim; //Índice de fim do bloco de elementos que a thread irá processar
...
void *somaElementos(void *arg) {
    ...
    //Alocar memória para armazenar a soma parcial
    float *soma_parcial = (float *)malloc(sizeof(float));
    *soma_parcial = soma;

    //Atualiza a soma parcial
    *(args->soma_parcial) = *soma_parcial;
    
    //Liberar a memória alocada para a soma parcial
    free(soma_parcial);
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < NTHREADS; i++) {
        ...
        if (args[i].fim > num_elementos) args[i].fim = num_elementos; //Garante que a última thread não ultrapasse o final do vetor
